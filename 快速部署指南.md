# 🚀 代码安全修复 - 快速部署指南

## 📋 部署前检查清单

- [ ] 已阅读《代码安全修复总结.md》
- [ ] 已备份当前运行的代码和数据库
- [ ] 已准备好数据库密码和邮箱密码
- [ ] 已确认服务器连接正常

---

## ⚡ 快速部署（3步完成）

### 步骤1: 设置环境变量（必须！）

```bash
# 在服务器上执行以下命令
# 将下面的密码替换为你的实际密码

export DB_USERNAME=root
export DB_PASSWORD=your_actual_db_password
export MAIL_USERNAME=your_email@qq.com
export MAIL_PASSWORD=your_mail_app_password

# 验证环境变量是否设置成功
echo $DB_PASSWORD
```

### 步骤2: 重新编译和部署后端

```bash
# 进入服务器目录
cd /home/nanqipro01/gitlocal/Introduction-to-Artificial-Intelligence/server

# 编译项目
./mvnw clean package -DskipTests

# 停止当前运行的服务（如果有）
# 方法1: 如果使用systemd
sudo systemctl stop ai-platform-server

# 方法2: 如果是直接运行的Java进程
# pkill -f "server-0.0.1-SNAPSHOT.jar"

# 启动服务（使用生产环境配置）
nohup java -jar target/server-0.0.1-SNAPSHOT.jar \
  --spring.profiles.active=prod \
  > server.log 2>&1 &

# 查看启动日志
tail -f server.log
```

### 步骤3: 重新部署前端

```bash
# 进入客户端目录
cd /home/nanqipro01/gitlocal/Introduction-to-Artificial-Intelligence/client

# 重新构建（如果npm依赖已安装，可跳过npm install）
npm run build

# 备份旧版本
sudo cp -r /var/www/html /var/www/html.backup.$(date +%Y%m%d)

# 部署新版本
sudo cp -r dist/* /var/www/html/

# 重启Nginx
sudo systemctl restart nginx
```

---

## ✅ 验证部署是否成功

### 1. 检查后端服务

```bash
# 检查服务是否运行
curl http://localhost:8082/api/chapters

# 查看日志
tail -n 50 server.log

# 检查安全响应头
curl -I http://localhost:8082/api/chapters | grep -E "X-Frame|X-Content-Type|X-XSS"
```

**预期结果**：
```
X-Frame-Options: SAMEORIGIN
X-Content-Type-Options: nosniff
X-XSS-Protection: 1; mode=block
```

### 2. 检查前端服务

```bash
# 访问前端
curl -I http://222.204.4.108/

# 检查前端文件
ls -la /var/www/html/
```

### 3. 测试安全功能

```bash
# 1. 测试TRACE方法（应该返回405）
curl -X TRACE http://222.204.4.108:8082/
# 预期: {"code": 405, "message": "HTTP方法不允许: TRACE"}

# 2. 测试TRACK方法（应该返回405）
curl -X TRACK http://222.204.4.108:8082/
# 预期: {"code": 405, "message": "HTTP方法不允许: TRACK"}

# 3. 检查所有安全响应头
curl -I http://222.204.4.108:8082/api/chapters
# 应该看到所有安全响应头
```

### 4. 在线安全检测

访问以下网站检查安全配置：
- https://securityheaders.com - 输入 `http://222.204.4.108`
- https://observatory.mozilla.org - 输入 `http://222.204.4.108`

**预期评分**：
- SecurityHeaders: B级或以上
- Mozilla Observatory: C+级或以上

---

## 🔧 常见问题解决

### 问题1: 启动后报数据库连接错误

**原因**: 环境变量未设置或MySQL未启用SSL

**解决方法**:
```bash
# 1. 检查环境变量
echo $DB_PASSWORD

# 2. 如果MySQL未配置SSL，临时使用开发环境配置
java -jar target/server-0.0.1-SNAPSHOT.jar \
  --spring.profiles.active=dev

# 3. 或者修改生产环境配置中的SSL设置
# 将 useSSL=true 改为 useSSL=false（临时方案）
```

### 问题2: 文件上传失败

**原因**: 上传目录权限不足

**解决方法**:
```bash
# 创建上传目录
sudo mkdir -p /var/www/ai-platform/uploads/avatars/

# 设置权限
sudo chown -R www-data:www-data /var/www/ai-platform/uploads/
sudo chmod -R 755 /var/www/ai-platform/uploads/
```

### 问题3: CORS错误

**原因**: 前端访问的域名不在白名单中

**解决方法**:
在 `WebConfig.java` 中添加你的域名：
```java
.allowedOrigins(
    "http://localhost:5173",
    "http://222.204.4.108",
    "http://your-domain.com"  // 添加你的域名
)
```

### 问题4: 邮件发送失败

**原因**: 邮件环境变量未设置

**解决方法**:
```bash
# 设置邮件环境变量
export MAIL_USERNAME=your_email@qq.com
export MAIL_PASSWORD=your_qq_app_password

# 重启服务
```

---

## 🎯 部署后检查项

### 必须检查（Critical）

- [ ] 后端服务正常运行（端口8082可访问）
- [ ] 前端页面可以正常打开
- [ ] 用户登录功能正常
- [ ] 数据库连接正常
- [ ] 环境变量已正确设置

### 安全检查（Important）

- [ ] HTTP安全响应头已生效
- [ ] TRACE方法已被禁用
- [ ] 文件上传安全验证生效
- [ ] CORS配置正确
- [ ] 错误页面不泄露详细信息

### 功能检查（Optional）

- [ ] 用户注册功能
- [ ] 头像上传功能
- [ ] 邮件发送功能
- [ ] 各章节内容访问正常
- [ ] 题库功能正常

---

## 📊 部署完成后的安全提升

### 代码层面（已完成）✅
- ✅ HTTP安全响应头保护
- ✅ 文件上传安全验证
- ✅ CORS配置加强
- ✅ 危险HTTP方法禁用
- ✅ 敏感信息环境变量化
- ✅ 生产环境安全配置

### 服务器层面（待配合《安全漏洞修复计划.md》完成）
- ⏳ Nginx升级（修复5个高危漏洞）
- ⏳ MySQL升级（修复1个中危漏洞）
- ⏳ SSH配置加固
- ⏳ 服务器版本信息隐藏

### 预期效果
- **代码安全性**: ⬆️ 显著提升
- **Web应用防护**: ⬆️ 从弱到强
- **整体风险**: ⬇️ 9.7/10 → 约7.0/10
- **配合服务器修复后**: ⬇️ 预计降至3.0/10以下

---

## 🔄 持久化环境变量（重要！）

为了防止服务器重启后环境变量丢失，需要将环境变量添加到配置文件：

### 方法1: 添加到systemd服务（推荐）

创建文件 `/etc/systemd/system/ai-platform-server.service`:
```ini
[Unit]
Description=AI Platform Server
After=network.target

[Service]
Type=simple
User=your_user
WorkingDirectory=/home/nanqipro01/gitlocal/Introduction-to-Artificial-Intelligence/server
Environment="DB_USERNAME=root"
Environment="DB_PASSWORD=your_password"
Environment="MAIL_USERNAME=your_email@qq.com"
Environment="MAIL_PASSWORD=your_mail_password"
ExecStart=/usr/bin/java -jar target/server-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod
Restart=always

[Install]
WantedBy=multi-user.target
```

然后启动服务：
```bash
sudo systemctl daemon-reload
sudo systemctl enable ai-platform-server
sudo systemctl start ai-platform-server
sudo systemctl status ai-platform-server
```

### 方法2: 添加到 ~/.bashrc 或 ~/.profile

```bash
# 编辑配置文件
nano ~/.bashrc

# 添加以下内容（文件末尾）
export DB_USERNAME=root
export DB_PASSWORD=your_password
export MAIL_USERNAME=your_email@qq.com
export MAIL_PASSWORD=your_mail_password

# 保存后重新加载
source ~/.bashrc
```

### 方法3: 使用.env文件（需要额外配置）

创建 `/home/nanqipro01/gitlocal/Introduction-to-Artificial-Intelligence/server/.env`:
```
DB_USERNAME=root
DB_PASSWORD=your_password
MAIL_USERNAME=your_email@qq.com
MAIL_PASSWORD=your_mail_password
```

在启动脚本中加载：
```bash
set -a
source .env
set +a
java -jar target/server-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod
```

---

## 📞 遇到问题？

如果部署过程中遇到问题：

1. **查看日志**
   ```bash
   # 后端日志
   tail -f /home/nanqipro01/gitlocal/Introduction-to-Artificial-Intelligence/server/server.log
   
   # Nginx日志
   sudo tail -f /var/log/nginx/error.log
   ```

2. **检查端口占用**
   ```bash
   sudo netstat -tlnp | grep 8082
   ```

3. **检查服务状态**
   ```bash
   sudo systemctl status nginx
   sudo systemctl status ai-platform-server
   ```

4. **回滚到之前版本**
   ```bash
   # 停止服务
   sudo systemctl stop ai-platform-server
   
   # 恢复备份
   sudo cp -r /var/www/html.backup.YYYYMMDD/* /var/www/html/
   
   # 使用旧版本jar
   # ...
   ```

---

## ✅ 部署完成确认

部署完成后，请确认：

- [x] 所有服务正常运行
- [x] 环境变量已持久化
- [x] 安全功能已验证生效
- [x] 用户功能正常
- [x] 性能无明显下降
- [x] 日志中无严重错误

**恭喜！代码层面的安全修复已完成部署！** 🎉

接下来请按照《安全漏洞修复计划.md》完成服务器层面的修复（Nginx升级、MySQL升级等）以达到最佳安全效果。

---

**文档版本**: v1.0  
**创建日期**: 2025-10-30  
**预计部署时间**: 30-60分钟

