...
.mode-btn {
  ...
  align-items: center;
  padding: 1.5rem;
  background: linear-gradient(135deg, rgba($info-color, 0.05) 0%, rgba($info-color-light, 0.05) 100%);
  border: 2px solid rgba($info-color, 0.1);
  border-radius: $card-radius;
  cursor: pointer;
  transition: all 0.3s ease;
  gap: 1rem;
  ...
  &:hover {
    transform: translateY(-2px);
    box-shadow: 0 8px 24px rgba($info-color, 0.2);
    border-color: rgba($info-color, 0.3);
  }
  
  &.active {
    background: linear-gradient(135deg, rgba($info-color, 0.15) 0%, rgba($info-color-light, 0.15) 100%);
    border-color: $info-color;
    box-shadow: 0 0 20px rgba($info-color, 0.3);
  }
}
